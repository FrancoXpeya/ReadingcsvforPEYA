function validateFile() {
    ...
    const fileNamePattern = /^[^\s]+_\d+$/;
    if (!fileNamePattern.test(fileName.split('.')[0])) {
        errorDisplay.innerHTML = '<p class="error-message">El nombre del archivo no cumple con el formato requerido. AquÃ­ un ejemplo de como debe ser "Catalogo_1234567".</p>';
        return;
    }
    ...
    reader.onload = function(e) {
        ...
        if (hasActive && hasQuantity) {
            errorHTML += '<p class="error-message">No puedes indicar las columnas â€œactiveâ€ y â€œquantityâ€ en un mismo archivo, debes indicar solo una de ellas.</p>';
            errorCount++;
        }
